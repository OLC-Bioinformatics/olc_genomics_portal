...
class GeneSeekrRequest(models.Model):
    ...
    seqids = ArrayField(models.CharField(max_length=64), blank=True, default=list)
    ...
    benchmark = models.CharField(max_length=48, default='', blank=True, null=True)
...
class GeneSeekrAzureRequest(models.Model):
...
class GeneSeekrDetail(models.Model):
    geneseekr_request = models.ForeignKey(
        GeneSeekrRequest,
        on_delete=models.CASCADE,
        related_name='geneseekrdetail',
        null=True)
    seqid = models.CharField(max_length=64, default='')
    # Pretty much identical to geneseekr request JSONField, but this one has percent ID for the
    # value instead of percent of times found.
    geneseekr_results = JSONField(default=dict, blank=True, null=True)
...
class TreeAzureRequest(models.Model):
    tree_request = models.ForeignKey(Tree, on_delete=models.CASCADE, related_name='azuretask')
